...
class CrPayrollTermination(models.Model):
    ...
    reference_locked_by_id = fields.Many2one("res.users", string="Referencia congelada por", readonly=True, copy=False)
    reference_locked_at = fields.Datetime(string="Fecha de congelamiento", readonly=True, copy=False)
    ...
    agreement_approved_by_id = fields.Many2one("res.users", string="Acuerdo aprobado por", readonly=True, copy=False)
    agreement_approved_at = fields.Datetime(string="Fecha de aprobación del acuerdo", readonly=True, copy=False)
    agreement_total = fields.Monetary(string="Total acordado", compute="_compute_agreement_totals", currency_field="currency_id")
    agreement_difference = fields.Monetary(string="Diferencia contra referencia legal", compute="_compute_agreement_totals", currency_field="currency_id")
    ...
    residual_receivable_move_id = fields.Many2one("account.move", string="Asiento de cuenta por cobrar", readonly=True, copy=False)
...
class CrPayrollTerminationAgreementLine(models.Model):
    ...
    termination_id = fields.Many2one("cr.payroll.termination", string="Liquidación", required=True, ondelete="cascade")
    currency_id = fields.Many2one(related="termination_id.currency_id", string="Moneda")
    component = fields.Char(string="Código del componente", required=True, readonly=True)
    name = fields.Char(string="Concepto", required=True, readonly=True)
    reference_amount = fields.Monetary(string="Importe legal de referencia", currency_field="currency_id", readonly=True)
    agreed_amount = fields.Monetary(string="Importe acordado", currency_field="currency_id")
    ...
    approved_by_id = fields.Many2one(string="Aprobado por", related="termination_id.agreement_approved_by_id", readonly=True)
