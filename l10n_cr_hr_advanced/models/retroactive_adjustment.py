...
class CrPayrollRetroactiveAdjustment(models.Model):
    ...
    name = fields.Char(string="Referencia", required=True, default="Ajuste retroactivo", tracking=True)
    ...
    amount_total = fields.Monetary(string="Diferencia total", compute="_compute_total", store=True, currency_field="currency_id")
...
class CrPayrollRetroactiveAdjustmentLine(models.Model):
    ...
    adjustment_id = fields.Many2one("cr.payroll.retroactive.adjustment", string="Ajuste retroactivo", required=True, ondelete="cascade")
    currency_id = fields.Many2one(related="adjustment_id.currency_id", string="Moneda")
    ...
    origin_period = fields.Char(string="Período origen", compute="_compute_origin_period", store=True)
    salary_rule_code = fields.Char(string="Código regla", readonly=True)
    concept_name = fields.Char(string="Concepto", readonly=True)
    original_amount = fields.Monetary(string="Importe original", currency_field="currency_id", readonly=True)
    recalculated_amount = fields.Monetary(string="Importe recalculado", currency_field="currency_id")
    difference = fields.Monetary(string="Diferencia", currency_field="currency_id", compute="_compute_difference", store=True)
