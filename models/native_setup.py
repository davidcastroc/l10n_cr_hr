...
class CrPayrollNativeSetup(models.AbstractModel):
    ...
    def setup_native_payroll(self):
        ...
        self._archive_legacy_aggregate_rules()
    ...
    def _archive_rules(rules):
        """Archiva reglas sin borrar históricos referenciados por recibos."""
        rules = rules.exists()
        if not rules:
            return

        if "active" in rules._fields:
            rules.write({"active": False})
        else:
            _logger.warning(
                "No fue posible archivar reglas salariales %s: "
                "el modelo no tiene campo active.",
                rules.ids,
            )
    ...
    def _cleanup_structure(self, structure, allowed_codes, base_rule=None):
        ...
        rules = self.env["hr.salary.rule"].sudo().with_context(
            active_test=False
        ).search(
        ...
        for rule in rules:
            ...
            if "active" in rule._fields and not rule.active:
                rule.active = True
    ...
    def _copy_rule_to_structure(self, source_rule, structure):
        ...
        if "condition_python" in source_rule._fields:
            values["condition_python"] = source_rule.condition_python
        ...
        for field_name in (
            ...
            if field_name in source_rule._fields:
                values[field_name] = source_rule[field_name]
    ...
    def _setup_structure_rules(self):
        ...
        common_codes = {
            ...
            "CR_VACATION_PAY",
            "CR_DISABILITY_DED",
            "CR_DISABILITY_EMP_TAX",
            "CR_DISABILITY_EMP_NONTAX",
            "CR_DISABILITY_SUBSIDY",
            "CR_COMMISSION",
            "CR_BONUS",
            "CR_AVAILABILITY",
            "CR_RETROACTIVE",
        ...
        source_rules = (
            ...
            .search(
                [
                    ("struct_id", "=", monthly.id),
                    ("active", "=", True),
                    ("code", "in", list(common_codes)),
                ],
                order="sequence, id",
            )
            .filtered(self._is_cr_rule)
        )
        ...
        for target in (biweekly, weekly, hourly):
            ...
            target_rules = self.env["hr.salary.rule"].sudo().search([
                ("struct_id", "=", target.id),
                ("active", "=", True),
            ])
        ...
        for structure, allowed_codes in special_specs.items():
            ...
            unwanted = rules.filtered(
                lambda rule: not (
                    self._is_cr_rule(rule)
                    and rule.code in allowed_codes
                )
            )
    ...
    def _setup_input_availability(self):
        ...
        for input_type in input_model.search([("code", "like", "CR_%")]):
            if input_type.code in settlement_codes:
                ...
                structure_ids = [
                    record.id
                    for record in (aguinaldo, settlement)
                    if record
                ]
            ...
            input_type.write({
                "struct_ids": [(6, 0, structure_ids)],
            })

    def _archive_legacy_aggregate_rules(self):
        Rule = self.env["hr.salary.rule"].sudo().with_context(active_test=False)
        legacy_codes = {
            "CR_SOC_EMP",
            "CR_SOC_PAT",
        ...
        self._archive_rules(legacy_rules)
    ...
    def _payroll_menus(self):
        ...
        result = []
        for imd in records:
            menu = self.env["ir.ui.menu"].sudo().browse(
                imd.res_id
            ).exists()
            if menu:
    ...
    def _pick_menu(candidates, tokens, require_parent=False):
        ...
        for xml_name, menu in candidates:
            ...
            score = sum(
                10 for token in tokens if token in xml_name
            )
            score += sum(
                3 for token in tokens if token in source_name
            )
