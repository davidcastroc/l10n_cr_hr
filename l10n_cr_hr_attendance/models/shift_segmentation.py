...
class CrPayrollShiftAnalysis(models.Model):
    ...
    name = fields.Char(string="Referencia", required=True, default="Análisis de jornada", tracking=True)
    ...
    total_presence_hours = fields.Float(string="Horas de presencia", compute="_compute_totals")
    total_paid_hours = fields.Float(string="Horas pagadas", compute="_compute_totals")
    total_ordinary_hours = fields.Float(string="Horas ordinarias", compute="_compute_totals")
    total_overtime_hours = fields.Float(string="Horas extra", compute="_compute_totals")
    total_recovery_hours = fields.Float(string="Horas de reposición", compute="_compute_totals")
    payroll_additional_total = fields.Monetary(string="Adicional de nómina", compute="_compute_totals", currency_field="currency_id")
    ...
    approved_at = fields.Datetime(string="Fecha de aprobación", readonly=True)
    ...
    def _compute_totals(self):
        for rec in self:
            ...
            rec.total_recovery_hours = sum(rec.line_ids.filtered(lambda l: l.classification == "recovery").mapped("paid_hours"))
    ...
    def action_calculate(self):
        ...
        for rec in self:
            ...
            # Horarios de reposición aprobados que se cruzan con esta marcación.
            # Se agregan como puntos de corte para que nunca se mezclen con horas extra.
            RecoveryLine = self.env["cr.payroll.hour.recovery.line"].sudo()
            recovery_lines = RecoveryLine.search([
                ("employee_id", "=", rec.employee_id.id),
                ("recovery_id.state", "in", ["approved", "partial"]),
                ("date_start", "<", rec.date_stop),
                ("date_stop", ">", rec.date_start),
            ], order="date_start,id")
            recovery_caps = {}
            for rline in recovery_lines:
                previous = rline.recovery_id.plan_line_ids.filtered(lambda x: x.date_start < rline.date_start)
                previous_actual = sum(previous.mapped("actual_hours"))
                cap = max((rline.recovery_id.hours_to_recover or 0.0) - previous_actual, 0.0)
                cap = min(cap, rline.planned_hours or 0.0)
                rstart = rec._to_local(rline.date_start)
                rstop = rec._to_local(rline.date_stop)
                points.update({rstart, rstop})
                if cap > 0 and rstart + timedelta(hours=cap) < rstop:
                    points.add(rstart + timedelta(hours=cap))
                recovery_caps[rline.id] = cap
            ...
            for a, b in zip(points[:-1], points[1:]):
                ...
                if not is_break:
                    for rline in recovery_lines:
                        rstart = rec._to_local(rline.date_start)
                        cap_stop = rstart + timedelta(hours=recovery_caps.get(rline.id, 0.0))
                        if recovery_caps.get(rline.id, 0.0) > 0 and a >= rstart and b <= cap_stop:
                            recovery_line = rline
                            break
                raw.append((a, b, is_break, rec._is_public_holiday(a), recovery_line))

            ordinary_remaining = rec.ordinary_paid_hours_limit
            rate = rec._hour_rate()
            monthly = rec.contract_id.cr_salary_mode != "hourly"
            for a, b, is_break, is_holiday, recovery_line in raw:
                ...
                if recovery_line:
                    Line.create({
                        ...
                        "multiplier": 0.0,
                        "payroll_additional": 0.0,
                        "recovery_id": recovery_line.recovery_id.id,
                    })
                    continue
        ...
        for rec in self:
            ...
            for line in rec.line_ids:
                ...
                Incident.create({
                    ...
                    "state": "approved",
                    "shift_analysis_id": rec.id,
                    "attendance_origin_period": "%s - %s" % (line.date_start, line.date_stop),
                })
            rec.state = "generated"
        return True
...
class CrPayrollShiftAnalysisLine(models.Model):
    ...
    analysis_id = fields.Many2one("cr.payroll.shift.analysis", string="Análisis de jornada", required=True, ondelete="cascade")
    currency_id = fields.Many2one(related="analysis_id.currency_id", string="Moneda")
    recovery_id = fields.Many2one("cr.payroll.hour.recovery", string="Reposición asociada", readonly=True)
    date_start = fields.Datetime(string="Inicio", required=True, readonly=True)
    date_stop = fields.Datetime(string="Fin", required=True, readonly=True)
    classification = fields.Selection([
        ...
        ("recovery", "Reposición de horas"),
    ], string="Clasificación", required=True, readonly=True)
    presence_hours = fields.Float(string="Horas de presencia", readonly=True)
    paid_hours = fields.Float(string="Horas consideradas", readonly=True)
    is_holiday = fields.Boolean(string="Es feriado", readonly=True)
    hour_rate = fields.Monetary(string="Tarifa por hora", currency_field="currency_id", readonly=True)
    multiplier = fields.Float(string="Multiplicador", readonly=True)
    payroll_additional = fields.Monetary(string="Adicional de nómina", currency_field="currency_id", readonly=True)
