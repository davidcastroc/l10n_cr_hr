...
class CrPayrollLateEvent(models.Model):
    ...
    name = fields.Char(string="Referencia", required=True, default="Evento tardío", tracking=True)
    ...
    registered_date = fields.Date(string="Fecha de registro", required=True, default=fields.Date.context_today)
    ...
    generated_leave_id = fields.Many2one("hr.leave", string="Ausencia generada", readonly=True, copy=False)
    ...
    def action_cancel(self):
        for rec in self:
            if rec.state == "applied":
                raise ValidationError(_("Un evento ya aplicado no puede cancelarse directamente. Debe revertir primero la ausencia generada y revisar cualquier nómina afectada."))
            rec.state = "cancel"
        return True
